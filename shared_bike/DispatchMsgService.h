/***********************************************************
* 负责分发消息服务模块，其实就是把外部收到的消息，转化为内部事件，
也就是data->msg->event的解码过程
* 然后再把事件投递到线池的消息队列，由线程池调用其process方法对事件进行处理，
最终调用每个event的handler方法
...
// 多线程相关 ：定义了线程池相关的变量和方法、互斥锁和条件变量
// 事件管理 ：定义了事件处理器的订阅和取消订阅方法、事件队列的管理等
// 响应事件队列 ：通过队列来统一管理响应事件，确保事件的有序处理
