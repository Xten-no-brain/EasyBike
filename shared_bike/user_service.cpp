...
bool UserService::exist(std::string & userName)
{
	...
	if (!sql_conn_->Execute(sql_content, record_set)) return false;
...
bool UserService::addBike(i32 bikeid, r64 longitude, r64 latitude)
{
	char sql_content[1024] = { 0 };
	sprintf(sql_content, 
		"INSERT INTO bikeinfo(devno, bike_point) VALUES(%d, GeomFromText('POINT(%lf %lf)'))", 
		bikeid, longitude, latitude);
